...
- `verify_canaster_email_otp` creates `mail_account` and `mail_box` rows with permission `569633` (`Owner: Read, Execute, Refer`) so later mail storage can use those rows as foreign-key targets for the same user.
