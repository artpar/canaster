...
mkdir -p /opt/canaster/bin /opt/canaster/data/storage

cat >/opt/canaster/bin/deploy-image.sh <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
chmod 0755 /opt/canaster/bin/deploy-image.sh

IMAGE="$(metadata instance/attributes/canaster-daptin-image || true)"
if [[ -n "$IMAGE" ]]; then
  /opt/canaster/bin/deploy-image.sh "$IMAGE"
fi
