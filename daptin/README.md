...
- `request_canaster_email_otp` on `user_account` accepts `email`, creates the user if missing, generates an OTP, and sends it through Daptin's `mail.send` SMTP performer.
