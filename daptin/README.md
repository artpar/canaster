...
8. Create one Daptin `site` row per frontend hostname. The current public frontend rows are `canaster.in` and `www.canaster.in`.
9. Keep `api.canaster.in` as the Daptin admin/API hostname and do not create a static site row for it.
10. CI builds the Daptin image, builds `dist/`, uploads it to the site storage prefix, deploys the image to the VM over IAP SSH, and probes the VM runtime.
