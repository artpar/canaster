...
- Verify the imported action row through `daptin-cli`; the `get_canaster_document_by_public_path` action must keep `permission=2085152` so routed-template guests can execute it without opening document CRUD. This is separate from `AccessGroups`, which provisions usergroup relations but does not grant guest execution.
